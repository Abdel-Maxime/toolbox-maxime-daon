...
function detectEncoding(buffer) {
  ...
  // Use jschardet on a binary string for reliable detection
  ...
  // Map jschardet names to TextDecoder-compatible names
  const map = {
    "ASCII": "UTF-8",
    "UTF-8": "UTF-8",
  ...
  for (const [key, val] of Object.entries(map)) {
    if (enc.includes(key)) { decoderName = val; label = key; break; }
  }

  // Confidence fallback: if low confidence & not UTF-8, try UTF-8 strict
